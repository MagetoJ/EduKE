...
const JWT_SECRET = 'your-super-secret-key-replace-this-now';
...
app.post('/api/parent/access', async (req, res) => {
  try {
    ...
    if (!studentUser) {
      res.status(401).json({ error: 'Student account not found or password not set.' });
      return;
    }
...
app.post('/api/users', async (req, res) => {
  try {
    ...
    const placeholders = ['?', '?', '?', '?', '?', '?'];
    const bindings = [name, email, passwordHash, role, phone ?? null, school_id ?? null];
...
app.get('/api/reports/performance-summary', (req, res) => {
  res.json([
...
app.get('/api/reports/school-analytics', (req, res) => {
  res.json([
